...
### Decisions update (2026-07-07) — Backlog scoping pass

Scoped the full open-issue backlog into implementation-ready tickets. The design decisions agreed during that pass:

| Topic             | Decision                                                                                                                                                                                                                                                                                                                    |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Spell economy     | Spells become **scroll loot** (epic #384). New spells via **curated recipes** (hand-authored data table, no RNG); **3 identical scrolls → +1 level**, max 3, **pure power scaling**. **Universal drop, class-locked use** (off-class scrolls are combine/scrap fodder). New characters **start with their current class kit** as L1 owned spells. Loadout changed **in town**, **persisted to the save** (save-format change). |
| Ability roster    | The ability one-liners (#131–138, #81–83) are **WoW-adapted** and **blocked on #373** (Phase 12 spell rework). A shared **`Minion` entity (#81)** underpins the Ranger pet, Raise Corpse, and pet-command abilities.                                                                                                          |
| Multiplayer       | **Online co-op only** — PvP (#3) dropped. **Peer-to-peer, host-authoritative** (WebRTC gameplay; **Vercel functions + KV broker only the signaling** handshake). **Scope-now / build-later** (epic #2): de-risk via a transport spike + a per-player-state refactor first.                                                     |
| Ascension (#58)   | **Branching choice at max level** (each base class → advanced subclasses). Reward is **unlocked spell schools + ascended-only spells/abilities** — spell access, not raw stats. Depends on the scroll economy (#384).                                                                                                          |
| Level-up (#1)     | **Automatic stat gains + a summary screen** (no point allocation). Note: leveling is currently cosmetic, so this adds the actual level reward too.                                                                                                                                                                            |
| Bane system       | Extend the stat-modifier-only `Banes` (#90) with **DoT ticks, an on-enemy visual, stacking rules, and `cleanup()`** (ties #328). **CC-unification deferred** to a follow-up. Bleed (#91) is the first concrete DoT, blocked on #90.                                                                                            |
| Known bugs        | #112 (cast doesn't start auto-attack) — fold into #373. #108 — bosses get a central **`cc_duration_mult`**. #109 — keep per-target crit and fix a **discovered Multishot self-damage** (base `castSpell(this.player)` path).                                                                                                   |
| Dropped / closed  | **Scoring** (#45/#46) — different direction. **PvP** (#3). **Dash/Jump mobility** (#5/#6) — not planned. **#10** superseded by the startup epic #377.                                                                                                                                                                         |
