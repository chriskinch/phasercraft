...
class Item {
	...
	round(stat: AdjustedStat): AdjustedStat {
		return {...stat, rounded: stat.format === "percent" ? Math.ceil((stat.adjusted + Number.EPSILON) * 100) / 100 : Math.ceil(stat.adjusted)} as AdjustedStat;
	}
	...
	getCategory(): string {
		return sample(["amulet", "armor", "axe", "bow", "gem", "helmet", "misc", "staff", "sword"]) as string;
	}
