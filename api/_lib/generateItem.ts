...
import type { ItemInput, AssignedStat } from "./types";

// fantasy-content-generator ships no types; the subset we use is declared here.
...
const getRandomCategory = (): Category => sample(Categories) as Category;
...
const allocateStatIterator = (pool: number, length: number) => {
    ...
    const statIterator = {
        next: (key: Stat) => {
            const range = 1 / (length * 2);
            const deduction = Math.round(pool * random(range, range * 3));
            if (count < length - 1) {
                pool -= deduction;
                count++;
...
export const generateItem = (data?: ItemInput) => {
    const qualityMap: QualityMap = getQualityMap();
    const generated = FCG.MagicItems.generate() as {
    ...
    const categoryMax: number = thing[category as keyof typeof thing];
